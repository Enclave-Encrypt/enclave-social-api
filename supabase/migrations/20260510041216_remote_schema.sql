  ...
  create table "public"."user_device_history_grants" (
    "id" bigint generated always as identity not null,
    "user_id" uuid not null,
    "user_device_id" bigint not null,
    "key_version_id" bigint not null,
    "grant_scope" jsonb not null default '{"kind": "full_history"}'::jsonb,
...
alter table "public"."user_device_history_grants" enable row level security;

CREATE UNIQUE INDEX user_device_history_grants_active_device_version_idx ON public.user_device_history_grants USING btree (user_id, user_device_id, key_version_id) WHERE (revoked_at IS NULL);

CREATE UNIQUE INDEX user_device_history_grants_pkey ON public.user_device_history_grants USING btree (id);

CREATE INDEX user_device_history_grants_user_active_idx ON public.user_device_history_grants USING btree (user_id, revoked_at);

alter table "public"."user_device_history_grants" add constraint "user_device_history_grants_pkey" PRIMARY KEY using index "user_device_history_grants_pkey";

alter table "public"."user_device_history_grants" add constraint "user_device_history_grants_created_by_device_id_fkey" FOREIGN KEY (created_by_device_id) REFERENCES public.user_devices(id) ON DELETE SET NULL not valid;

alter table "public"."user_device_history_grants" validate constraint "user_device_history_grants_created_by_device_id_fkey";

alter table "public"."user_device_history_grants" add constraint "user_device_history_grants_user_device_id_fkey" FOREIGN KEY (user_device_id) REFERENCES public.user_devices(id) ON DELETE CASCADE not valid;

alter table "public"."user_device_history_grants" validate constraint "user_device_history_grants_user_device_id_fkey";

alter table "public"."user_device_history_grants" add constraint "user_device_history_grants_user_id_fkey" FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE not valid;

alter table "public"."user_device_history_grants" validate constraint "user_device_history_grants_user_id_fkey";

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.setup_primary_history_device(p_user_device_id bigint, p_wrapped_history_root_key text, p_wrapping_public_key text DEFAULT NULL::text)
 RETURNS TABLE(trust_id bigint, key_version_id bigint, key_version integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
declare
  ...
  inserted_trust_id bigint;
  inserted_key_version_id bigint;
begin
  if auth.uid() is null then
    raise exception 'Not authenticated';
  end if;

  if nullif(trim(p_wrapped_history_root_key), '') is null then
    raise exception 'Wrapped history key is required';
  end if;
  ...
  if device_owner is distinct from auth.uid() then
    raise exception 'Device does not belong to current user';
  end if;

  if exists (
    select 1
    from public.user_device_trusts t
    where t.user_id = auth.uid()
      and t.trust_level = 'primary'
      and t.revoked_at is null
  ) then
    raise exception 'Primary device already exists';
  end if;

  select coalesce(max(version), 0) + 1
    into next_version
  from public.user_history_key_versions
  where user_id = auth.uid();

  insert into public.user_history_key_versions (
    user_id,
    version,
    status,
  ...
  returning id into inserted_key_version_id;

  insert into public.user_device_trusts (
    user_id,
    user_device_id,
    trust_level,
    can_approve_devices,
  ...
  returning id into inserted_trust_id;

  insert into public.user_device_history_grants (
    user_id,
    user_device_id,
    key_version_id,
    grant_scope,
  ...
  values (
    auth.uid(),
    p_user_device_id,
    inserted_key_version_id,
    '{"kind":"full_history"}'::jsonb,
    p_wrapped_history_root_key,
    p_wrapping_public_key,
    p_user_device_id
  );

  return query
  select inserted_trust_id, inserted_key_version_id, next_version;
end;
$function$
;

CREATE OR REPLACE FUNCTION public.apply_platform_key_purchase(p_user_id uuid, p_quantity integer, p_stripe_customer_id text, p_checkout_session_id text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.get_server_settings_context(p_server_id bigint)
 RETURNS TABLE(id bigint, display_name text, description text, category text, visibility text, rules text, welcome_message text, banner_url text, icon_url text, invite_code text, show_posts_in_global_feed boolean, monetization_enabled boolean, theme_enabled boolean, theme_id text, appearance_preferences jsonb, use_server_theme boolean, my_role text, nickname text)
 LANGUAGE sql
 STABLE
 SET search_path TO 'public'
AS $function$
...
grant delete on table "public"."user_device_history_grants" to "anon";

grant insert on table "public"."user_device_history_grants" to "anon";

grant references on table "public"."user_device_history_grants" to "anon";

grant select on table "public"."user_device_history_grants" to "anon";

grant trigger on table "public"."user_device_history_grants" to "anon";

grant truncate on table "public"."user_device_history_grants" to "anon";

grant update on table "public"."user_device_history_grants" to "anon";

grant delete on table "public"."user_device_history_grants" to "authenticated";

grant insert on table "public"."user_device_history_grants" to "authenticated";

grant references on table "public"."user_device_history_grants" to "authenticated";

grant select on table "public"."user_device_history_grants" to "authenticated";

grant trigger on table "public"."user_device_history_grants" to "authenticated";

grant truncate on table "public"."user_device_history_grants" to "authenticated";

grant update on table "public"."user_device_history_grants" to "authenticated";

grant delete on table "public"."user_device_history_grants" to "service_role";

grant insert on table "public"."user_device_history_grants" to "service_role";

grant references on table "public"."user_device_history_grants" to "service_role";

grant select on table "public"."user_device_history_grants" to "service_role";

grant trigger on table "public"."user_device_history_grants" to "service_role";

grant truncate on table "public"."user_device_history_grants" to "service_role";

grant update on table "public"."user_device_history_grants" to "service_role";


  create policy "Users can read their history grants"
  on "public"."user_device_history_grants"
  as permissive
  for select
  to authenticated
